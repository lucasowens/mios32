...
mental state; the [reference catalog](../apps/sequencers/midibox_seq_v4/doc/MBSEQV4_REFERENCE.md) owns derived facts about
...
8. **Validate the system, not the fragment.** *(Added 2026-06-21 — the strategy shift;
   ...
   *emergent property of the whole flow*, not of a part; and locally-correct fragments keep
   ...
   - **Method:** lay the system flat first — a budget map, a control-surface map, a flow/dirty
     ...
     friction**, not from a feature catalog. First flat-map: `doc/plans/archive/2026-06-21-system-flatmap.md`.
...
performable** — see §8 + `doc/plans/archive/2026-06-21-system-flatmap.md`.*
...
| Recall-freeze cure (#1) | shipped 2026-06-22; stale-cited as open, reconciled 2026-07-10 (regression pin `test_recall_freeze_cure.py`) | §8, §10 |
...
| Operating grammar / rack (PROC page; ChordMask/Echo/Groove/LFO/Robotize/PitchGen/TrigGen tenants) | **shipped — G0→G2 face-hook fold, 2026-07-03→2026-07-08; base-layout redesign + merges → 11 rows, 2026-07-12/13** | §9 |
| Voicing tenant (Sprd/Inv/Drop/Strm/Tilt + chord-GRAVITY collapse ladder) | **shipped — by-ear GO 2026-07-11/12, HIL 268/268** | §9 |
| **Slicer tenant act 1** (chop the HEARD loop: Grid/Seed/Str/Rept/Rev + SlcOr painted order) | **shipped — by-ear GO 2026-07-16, HIL 278/278 baseline** | §9 |
| **Slicer act 2** (Chok/Motn/1stp + jump pads + ORDR plane: paint/punch-chop/interject/read-head) **+ 4 capture fixes** (ext-CC travel, FTS field membership, drum transpose travel, drum tape per-instrument) | **BUILT 2026-07-17/18 — by-ear + HIL pass pending** (OPEN_ITEMS "Pending validation") | §9, OPEN_ITEMS |
...
[REFERENCE §3](../apps/sequencers/midibox_seq_v4/doc/MBSEQV4_REFERENCE.md) for the bus deep-dive.
...
- A saved pattern persists trg/par layers + track CCs **0x80–0x95**
  ...
  capacity (~75 KB for ~8 used slots). See [REFERENCE](../apps/sequencers/midibox_seq_v4/doc/MBSEQV4_REFERENCE.md) for the byte map.
...
- [MBSEQV4_REFERENCE.md](../apps/sequencers/midibox_seq_v4/doc/MBSEQV4_REFERENCE.md), [MBSEQV4_MANUAL_FORK.md](../apps/sequencers/midibox_seq_v4/doc/MBSEQV4_MANUAL_FORK.md),
  [MBSEQV4_TODO_TRIAGE.md](../apps/sequencers/midibox_seq_v4/doc/MBSEQV4_TODO_TRIAGE.md) (the last is housekeeping, *not* the
  musical roadmap).
...
> follows is the roadmap the first **flat-map** produced (`doc/plans/archive/2026-06-21-system-flatmap.md`).
...
spec live in `doc/plans/archive/2026-06-21-system-flatmap.md`.
...
`doc/plans/2026-06-09-tension-workbench.md` (plan = scaffolding; that file
was executed and deleted — outcome in the [DECISIONS LOG](MBSEQV4_DECISIONS_LOG.md), 2026-06-09).
...
Two layers (split 2026-07-11). **This section = decisions in force**: the curated
thematic groups below, revised only with a dated note (strikethrough + pointer when
retired). **The dated session chronology** lives in
[MBSEQV4_DECISIONS_LOG.md](MBSEQV4_DECISIONS_LOG.md) — append new session blocks
there and add one line to the chronology index at the end of this section.
...
### Chronology — the dated session record (full text: [MBSEQV4_DECISIONS_LOG.md](MBSEQV4_DECISIONS_LOG.md))

> Moved out 2026-07-11 to keep Part I readable. One line per block, newest last;
> the LOG holds every block verbatim. **New sessions append the full block to the
> LOG and add one line here.**

- **2026-06-09 — Tension Workbench**: direction confirmed; §2.7 adopted (unit of validation = workflow bundle); §9 entries stay decision-sized
- **2026-06-10 — Track 2 BUILT**: pitch-chain migration into the render stack (HIL 108/108)
- **2026-06-11 — Save model, groups, phrases**: D1–D10 direction reversals; recall grain = track; tape replaces wrap-oldest quick-capture
- **2026-06-13 — PHRASES cross-session occupancy + "never lose work" recall** (SHIPPED)
- **2026-06-14 — PHRASES Stage B-rest**: drift signal + naming (SHIPPED)
- **2026-06-14 — Live-surface hardening**: by-ear-safety pass (phantom-pull guard, partial-capture occupancy, SHADE persist, …)
- **2026-06-16 — POSTURE-MORPH Loop A + recall landing feel** (SHIPPED; HIL 159/159)
- **2026-06-19 — Three-tier "return" model + durable SET baseline + the generative freeze/bounce law** (DECIDED, design-ahead; includes the "Provisional — NOT committed (Part II)" sub-block)
- **2026-06-20 — CAPTURE and BOUNCE are one verb**: unified retroactive FREEZE → anywhere (model decided); precise gate + multi-step length shipped toward it
- **2026-06-21 — Strategy shift: validate the SYSTEM, not the fragment** (§2 #8 added)
- **2026-06-22 — Phrase mode + morphing reviewed under the capture-centric model**: recall-freeze cured (DRIFT-gated writeback), morph kept
- **2026-06-22 (cont.) — by-ear morph-feel tuning + lean capture** + capture-while-performing question opened
- **2026-06-23 — Capture-while-performing freeze premise was WRONG**: audible clock never froze; residual = ~640 ms UI hang (HIL 196/196)
- **2026-06-23 (cont.) — the ~640 ms control-surface hang FIXED**: MUTEX_SDCARD contention, lock-free REQ pre-check (HIL 197/197)
- **2026-06-23 — Play-readiness net Stage 2a**: unified UNDO/REDO journal SHIPPED (§8 queue #3)
- **2026-06-24 — Stage 2b: REVERT-undoable SHIPPED**; safety-net bundle complete (HIL 210/210)
- **2026-06-26 — Render change-detection SHIPPED**: all-16 GRAVITY render wall gone (static field renders zero; worst case CPU ~27%)
- **2026-06-26 (cont.) — config-grain self-bus ALREADY EXISTED** (TK 2019, Ctrl par-type); FREEZE/CAPTURE faithfulness code-traced + by-ear GO
- **2026-06-26 (cont.) — CAPTURE lifted off the one-measure constraint** (any-length grab while playing; HIL 216/216)
- **2026-06-28 — Unified CAPTURE page → the CANVAS model, chord-aware** (by-ear GO)
- **2026-06-28 — phrase_drift leak in the three slot-capture verbs FIXED**
- **2026-06-28 — As-heard windowing SHIPPED**: Phase GRID/HEARD toggle (HIL 234/234)
- **2026-06-28 — Foreign-clkdiv CAPTURE geometry fixed**: synch period-doubling + tick-based whole-measure classify (HIL 241/241)
- **2026-07-03 — G0: the operating grammar, proven on ChordMask** (SHIPPED)
- **2026-07-05 — G1: rack onto a param-list grammar; PROC a real page** (SHIPPED)
- **2026-07-05 — G1.5: emission FX join via EXPOSE-IN-PLACE; Echo the reference tenant** (SHIPPED)
- **2026-07-05 — ChordMask gains a static Self mask; EDIT page anchors on SOURCE** (durable rule)
- **2026-07-05 — G1.6: Groove, 2nd emission tenant, paintable 16-step shape** (SHIPPED)
- **2026-07-05 — G1.7: LFO, the rack's first modulation source** (SHIPPED)
- **2026-07-06 — G2 part 1: the PLANE toggle, Robotize as the two-faced tenant** (SHIPPED)
- **2026-07-07 — G2 part 2: PitchGen, the rack's first GENERATOR row** (SHIPPED)
- **2026-07-07 — G3: the trigger Turing machine** (independent trigger key-space; SHIPPED)
- **2026-07-08 — G2: formatter/defaults registry** collapses tenant duplication (SHIPPED)
- **2026-07-08 — G2: ChordMask/Groove/LFO GP-row surfaces folded into the face hook** (SHIPPED)
- **2026-07-08 — Tension row: GRAVITY viz ported in + zone-jump gesture** (SHIPPED)
- **2026-07-08 — Self-bus legibility, first cut**: three Ctrl targets decoded to units (SHIPPED)
- **2026-07-08 — Humanize joins the rack** (5th emission tenant; shipped-broken bypass bit found + fixed same session)
- **2026-07-09 — Arp joins the rack**: deterministic arpeggiator born as a render-stack slot, NOT the legacy playmode (SHIPPED)
- **2026-07-10 — Capture deposit: silent-park + the Fit×Phase length model** (SHIPPED)
- **2026-07-10 — Reconciliation: recall-freeze cure was ALREADY shipped 06-22**; stale citation fixed, regression pin added
- **2026-07-10 — Robotize → render-stack: DEFERRED BY CHOICE** (recon complete, not built)
...
- **2026-07-12 (cont. 4) — Ptch merge + PROC base layout** (tab-grid LCD mock convention adopted; rack 13→12: ChordMask row dissolved into the 8-dial Ptch cockpit Semi/Oct/Str/Bus/FTS/Scle/Root/Deg; fixed 12-slot mask keyboard — active PCs as names, inactive as dots, always shown, visible==paintable; identity right-aligned `Ptch  1/12 G1T1` + per-row abbrs; Ptch dbl-tap = reset + drop playmode; BUILT clean, flash/HIL/by-ear pending)
- **2026-07-12 (cont. 5) — Voic merge** (rack 12→11: Limit row dissolved into Voicing, merged row MOVED to position 2 — pitch-space rows cluster at top; dial bank Sprd/Inv/Drop/Strm/Tilt · SPACER gap · Lo/Hi; new PROC_KIND_SPACER blank cell; first cross-rowkind merge — EMISSION row fronts the LIMIT stack slot via the RowState arm; dbl-tap = voicing bypass + Lo/Hi→0 on the off edge; row-0 `Rng lo..hi` note-name readout when clamped; BUILT clean, flash/HIL/by-ear pending)
- **2026-07-12 (cont. 6) — Voic clamp decoupled + Hi rests at 127** (reverses cont. 5's dbl-tap draft: range clamp INDEPENDENT of the bypass — the 0x80 bit only ever gated voicing in the DSP, UI coupling removed; new PROC_KIND_LIMIT_HI presents the stock 0=open-top encoding as a top-detent 127 — read 0→127/write 127→0, deflt 127, zero seq_core/persistence delta; resting range reads Lo 0..Hi 127 = full keyboard; BUILT clean, flash/HIL/by-ear pending)
- **2026-07-12 (cont. 7) — Tension row expansion** (no merge — row grows: Grip/Grav/Shade·gap·FTS and moves to position 3, pitch-space cluster = Ptch/Voic/Tens; Shade = GRAVITY page's brightness ladder as a rack dial via PROC_KIND_SHADE + ShadePosGet/Name/Set exports, write<0 no-op + deflt -1 so push/reset never yank the global scale; FTS DOUBLED from Ptch — first deliberately-duplicated dial; zone name left-anchored col 41, grip bar unlabeled cols 24-39; BUILT clean, flash/HIL/by-ear pending)
- **2026-07-12 (cont. 8) — PGen/TGen screen sets** (Roll moved to cell 8 and DOUBLED onto both planes — one physical dice encoder, no plane flip needed; TGen mirrored for sibling symmetry; plane cue grew into the row-0 readout at col 41 as `OPER 1/2`/`STEP 2/2`/`LOOP 2/2` replacing the bare col-60 cue; Status_Gen's Up/Dn hint dropped; PGen moved to position 4 per the mock's `4/12` — rack now Ptch/Voic/Tens/PGen/Arp/Echo/Grv/LFO/Robo/TGen/Hum; BUILT clean, flash/HIL/by-ear pending)
- **2026-07-12 (cont. 9) — ARP row dissolves into TGen, TGen to position 5** (rack 11→10 = Ptch/Voic/Tens/PGen/TGen/Echo/Grv/LFO/Robo/Hum — the pitch/generative cluster leads; TGen OPER = Dens Rate Dpth ·· Arp Bus Roll, Arp CCs via SEQ_CC_Set→ArpSlotSync; TGen row ORs the ARP stack slot; dbl-tap stays ENGAGE⇄DISENGAGE, armed arp keeps playing — kill via Arp→Off; mock's "OPER 2/2" slip shipped as STEP 2/2; BUILT clean, flash/HIL/by-ear pending)
- **2026-07-13 (cont. 10) — Grve full-page cockpit + hold-step** (all 8 cells: Styl Intn Glob Sync Stps Lane Val Clr, position 6; Glob = stock global-groove flag live on the rack — Styl/Intn/Sync broadcast to the global set; Val = bipolar paint brush (0=intensity-follow) + HOLD-STEP editor — hold a GP step and turn Val for that step's exact cell, user picked it over a 2nd EDIT plane; Clr = whole-template reset; broadcast×seed stomp caught in review → proc_in_seed single-track guard; cont. 4-9 batch got its by-ear GO, HIL deferred to fold this in; BUILT clean)
- **2026-07-13 (cont. 11) — hold-step refined on first touch** (toggle only on QUICK release <350ms — holding to peek the Val cell then releasing changes nothing; toggle-off/erase SHADOWS the step's value and a re-tap RESTORES it, shadow beats brush, keyed to one template, cleared on style switch/Clr; gesture matrix: tap=toggle·hold=peek·hold+turn=value·hold+push=erase; BUILT clean)
- **2026-07-13 (cont. 12) — Humn to position 7** (page already right; abbr Humn, status col 41; feel pair Grve+Humn adjacent — rack: Ptch/Voic/Tens/PGen/TGen/Grve/Humn/Echo/LFO/Robo; BUILT clean)
- **2026-07-13 (cont. 13) — Robo to position 8** (both planes already matched the mock; Up/Dn=LOOP hint kept; status col 41; rack: Ptch/Voic/Tens/PGen/TGen/Grve/Humn/Robo/Echo/LFO — feel trio together, Echo/LFO close the tail; BUILT clean)
- **2026-07-13 (cont. 14) — Echo as-is; LFO → full stock set on CONF/DEST planes; REDESIGN COMPLETE** (Echo mock == current page, zero delta; LFO grows from the simplified G1.7 row to the full FX_LFO parameter set — CONF Wave/Amp/Phas/Step/Rst/1Sht/ClkD + palette face, DEST Note/Vel/Len/CC flags + xCC#/xCC/Offs/PPQN; Targ single-select router retired, flags independent; first NAMED planes via p1name/p2name; Step/Rst reach stock 0..255; Status_LFO deleted; all 10 rows now mocked or confirmed; BUILT clean, flash/HIL/by-ear pending)
- **2026-07-13 (cont. 15) — PROC datawheel = track walker** (turn = track ±1, push = group jump G1→…→G4 keeping position; the rack becomes a MATRIX — focused row/plane survive the switch; wheel previously duplicated GP1's headline ride, push was free; user idea + follow-up, endorsed; BUILT clean)
- **2026-07-13 (cont. 16) — gen STEPS activity strip** (PGen/TGen lock planes render the window's truth at row-1 cols 64-79: o=sounding/unlocked, #=sounding+locked, -=locked silence, .=empty; active=GATE; punch in → lock by sight → generate around the pinned phrase; BUILT clean)
- **2026-07-13 (cont. 17) — gen STEPS LEDs duo-color** (color 1 = triggered steps — visible pre-ENGAGE; color 2 = locks via new ui_gp_leds2 overlay on the pos-marker channel, XOR playhead; LCD strip kept; BUILT clean)
- **2026-07-13 (cont. 18) — pre-ENGAGE locks via ADOPT** (finding: fresh ENGAGE seeds a random line over the track — punched-in phrases never survived it; new SEQ_GENERATOR_Adopt/TrgAdopt allocate a DISENGAGED slot whose loop copies the SOURCE (SEQ_PAR_GetSource, not the mirror), lock taps auto-adopt; engage of an adopted slot RE-adopts the then-current source + arms undo; `adopted` = parallel CCM array, struct size frozen for gen-state/SlotSet; BUILT clean — closes the pre-HIL batch cont. 10-18)
- **2026-07-15 — Slicer tenant act 1** (chop the HEARD loop into 2/4/8/16-step slices + resequence: render-stack TAIL slot 5, buffer PERMUTE — non-bijective map ⇒ reorder/stutter/reverse one primitive, bounce-faithful for free; dials Grid/Seed/Str/Rept/Rev on new CCs 0xA4-0xA8, 0-neutral ⇒ V5 as-is; painted order = new SlcOr par type 25, Waypoint idiom, EDIT-page painted, always wins over Seed; Str thermometer = painted lower half / seeded upper; rack row 11 "Slic" pos 6, double-tap = config-preserving bypass GRID bit7; sweep fence → quiet path; BUILT clean, flash/HIL/by-ear pending; plan `doc/plans/2026-07-15-slicer-tenant.md`)
- **2026-07-16/17 — Slicer by-ear GO + 10 HIL pins + the ext-CC replay gap; baseline 278/278** (by-ear "works!" after an EDIT "????"-display fix; 14 capture-family reds = NOT capture bugs but the jam's armed slicer SURVIVING every V3/V4-record load — the ext-CC replay only wrote CCs present in the record; fix `ExtCcNeutralExtend` neutral-extends pattern/track loads to the live count, the PhraseReadCCs contract applied to loads; DURABLE RULE: a new ext CC isn't 0-neutral-safe until the READ side extends old records; sharp edges boarded: CMD_BANK_CREATE stale bank cache, harness stop no-rewind; full block in DECISIONS_LOG)
- **2026-07-17 — Slicer act 2: choke + motion + jump pads** (all four act-1 deferred threads picked; three BUILT: **Chok** CC 0xA9 = edge-cut thermometer clearing GLIDE ties at choked slices' final step — the only render-reach cross-edge tail (par Length clamps at 96 ticks), painted NOT skipped, works at Str 0; **Motn** CC 0xAA = per-bar re-roll of the seeded fill, value quarters 8/4/2/1 bars, epoch=measure_ctr/N folded into the SEED axis only, boundary renders SYNCHRONOUSLY from the ref_step==0 hook so the chop lands ON the One (RenderTracks runs before the hook — a dirty flag alone is one tick late); **jump pads** = PROC_FACE_SLICE_JUMP face1, GP9-16 punch the playhead to slice 1-8 via existing SEQ_CORE_SetTrkPos manual_step latch = step-quantized, active on geometry regardless of bypass; 0xA9/0xAA degrade-safe in V5 (write side always zero-clamped headroom + ExtCcNeutralExtend covers V1..V4), NO V6; BUILT clean, flash/by-ear/HIL pending; plan `doc/plans/2026-07-17-slicer-act2.md`)
- **2026-07-17 (cont.) — Slicer ORDR paint plane, first-instinct build** (user skipped the mock ritual — "give your first instinct a try, then ill refine"; instinct = **encoders paint / buttons jump / LCD shows the RESOLVED map**: 16 cells, painted bare vs machine-decided in parens + `<` = REV — seed-browsing animates only the parens cells, the painted skeleton visibly stays put; turn = set 1..S / 0 = unpaint, push = unpaint, GP = jump-audition, first turn AUTO-ADOPTS a SlcOr layer (drum fenced to TrkEvnt); **first PURE-FACE plane 2** (face2 + params2 NULL — HasPlane2/SlotParams widened, empty param list never falls through to plane 1); slice_map_build → CALLER buffers, new SEQ_CORE_SlicePreview (16-entry UI locals, race-free, S'=min(S,16) IS the real first window) + SliceOrderPaint (zero the slice span, then first step); CHOP readout compacted to col 50, cue owns 41; full block in DECISIONS_LOG)
- **2026-07-18 — Slicer act-2 refinement day: punch-chop trio + two capture bugs** (same-day refinements from hands-on: **Motn → 5-detent selector** (0..127 thermometer read "stuck at 8bar", GRID idiom now); **1stp grid** (step-grain break chopper, GRID values REMAPPED 1..5 = 1/2/4/8/16, REV inert at 1stp, pins shifted); **punch-chop** (recorded chop IS a painted order — ORDR pads = SOURCES, quantized punch paints the landing; no event recording, bar stays locked); **momentary interject** (press/tap/hold: press sounds K at once via a TRANSIENT map override, quick tap = punch commit, hold = repeat + free return — the playhead never left; performance ladder slip/interject/record complete); **read-head wink** (ORDR wink = map[current position], the permute's virtual playhead bouncing on the LEDs — wink-meaning matches pad-meaning per plane); then the capture→switch "sounds different" hunt found **the ext-CC travel gap** (every capture/bounce/copy CC loop stopped at 0x7F — voicing/slicer dials never travelled; SEQ_CORE_CC_INHERIT_COUNT 0xB0 + CcInherit, 9 sites) and **FTS now survives the bounce reset** (idempotent on baked tape; the copy stays a member of the global harmonic field — capture freezes generation, not field membership); then the user's drum-track facts nailed two more: **drum transpose travels through capture** (drum steps store NO notes — gates+vel per instrument, pitch = per-drum config + emission transpose — so the config IS the pitch and must TRAVEL; chord re-apply idiom generalized to all 4 flatten sites; RULE: material can't carry pitch → config travels, material carries it → baked+reset) and **drum tape capture got per-instrument write-back** (the tape sink was melodic-mono by construction — every drum funnelled into instrument 0; tape = wire tap with no instrument identity, so the grab now maps events back via an expected-emitted-note table: lay_const row A + Semi/Oct sign-decode + FTS snap, nearest fallback; stopped drum re-sim stays mono/out-of-scope); full blocks in DECISIONS_LOG; validation state → OPEN_ITEMS "Pending validation")
- **2026-07-18 (cont. 7) — Capture-fidelity determination + the flatten family stops over-resetting** (deep-dive audit of every capture path; **the two-family LAW: FLATTEN deposits (mirror copies — ToSlot/ToTrack/ToSlotTrack/chord-route/ProcessorBounce) hold render-stack output only, so deterministic emission shapers are PRESERVED on the dst; TAPE deposits (live tape + re-sim) hold the emitted stream, so the same shapers are RESET (baked in the notes); generation resets in both**; shipped: reset split `SEQ_CC_ResetGenerativeForFlatten/ForTape` — flatten now keeps echo/LFO/fx-dupl/SUSTAIN/synch-to-measure/deterministic direction + the Delay/Roll/Nth/Root/Scale lane assignments (closes "copies strip Echo/LFO"); `SEQ_CORE_CaptureCopyStreamLanes` — CC/PB/PC/AT lanes now reach span deposits from the source mirror, window-rotated (Ctrl lanes deliberately not copied); **drum tape pitch bake** — kits with a Note par lane get the taped pitch stamped per (step,instrument), Semi/Oct travel undone there (lane carries it), + taped gate → Length lane; **chord fence** on the RAM grab (-13; slot verb keeps the index route); open forks boarded: groove-on-tape by-ear, drum→note-track deposit verb, re-sim drum sink port, RAM chord window, drum LIMIT travel; zero-warning build, no existing pin asserts the old contract; full block in DECISIONS_LOG)
- **2026-07-18 (cont. 8-12) — Capture-fidelity VALIDATED → by-ear GO; suite 287/287 = new baseline** (deterministic instrumentation first — diag_capture_fidelity diff table, kit-init verb 0x41, 7 pins → 285/285 + the user-kit slot diff byte-identical; then the EAR overruled the green suite: emitted-stream A/B on the T5 kit caught the grab model's hard-clamp vs emission's octave-fold (nearest-match merged a drum's rows) + the stopped re-sim instr-0 collapse; fixes = shared `capture_drum_expected_notes` now uses SEQ_CORE_TrimNote — grab-time models must call the emission chain's OWN primitives — + per-instrument re-sim drum write-back; +2 pins → 287/287, A/B MATCH both transports, by-ear GO "that got it!"; gotcha: a reboot reloads session MUTE state — an emission probe reading 0 events ⇒ check muted_mask first)
- **2026-07-18 (cont. 13) — Robotize rack regression + all-zero dial defaults** (the 07-08 registry refactor had routed ROBO_PROB's engage-seeds onto the row's own PROBABILITY CCs: Note/Oct dials jumped on engage, Vel/Len (32) truncated in the 5-bit fields, and the RANGES stayed unseeded → robotize silent on any fresh-engaged track; fix = direct range-seed writes restored in the headline case + RULE: SeedRowDefaults only seeds a row's OWN params — off-row seed targets (Robotize ranges, Humanize mode byte) seed directly; second decision: per-dim probabilities default 0 = pass-through in SEQ_CC_Init AND ResetGenerativeCommon — stock full-31 gone, Prob arms silently, each dimension swept from 0 on its own dial; by-ear GO, no pin possible (encoder-level path))
- **2026-07-19 — Kit mode: whole-kit drum overview on the TPD** (brainstorm "see all drums at once" → 3 same-day hardware iterations → landed as an OVERLAY not a screen: **hold EDIT → GP5 "Kit"** on drum tracks keeps the stock step-view LCDs + stock programming; the **TPD becomes the kit grid** — 16-step window × 8-drum window, green = gate, selected row pulses, playhead = inverting green tracer column, window follows selection on big kits — and the **datawheel (+Up/Down) scrolls drums** (intercept must sit ABOVE the datawheel-mode dispatch); two durable bugs found+fixed en route: **LCD buffer bit 7 = transfer flag** (chars ≥0x80 never render, stale cells show through — 0xFF ROM blocks are unusable, use VBars char 7 / PrintVBar; in auto-memory) and the **promiscuous EDIT-RECORDING learn hook** (any CC/AT/PB or stray NoteOff on a record port converted a step-tap into an overwriting punch-in; now note-gesture-only, loopback ports excluded, panel pads unaffected); runtime-CGRAM infra (`SEQ_LCD_InitRuntimeSpecialChars`, content-cached) kept caller-less for future LCD bitmap ideas; by-eye GO "one of the most useful features we've added yet"; full block in DECISIONS_LOG)
...
> standing forks: genuinely-open questions and deliberately-deferred builds with their revive
> conditions. Closed/shipped entries move verbatim to the
> [DECISIONS LOG](MBSEQV4_DECISIONS_LOG.md) ("Migrated from §10" section); day-to-day
> defects/housekeeping live on [OPEN_ITEMS.md](OPEN_ITEMS.md), not here. Pruned 2026-07-11.
...
**STOPPED multi-bar CAPTURE — DEFERRED BY CHOICE (2026-07-10).** While-PLAYING any-length
grabs already work note-for-note; only the STOPPED re-sim stays one-global-measure. Revive
only if stopped long-loop grabs become a wanted workflow — reopening = the ring re-frame +
the drive-hang debug (two walls). Full text + revive conditions → LOG; postmortem:
...
**Save-model rethink (2026-06-11) — SHIPPED as FEARLESS + RECOMBINE** (final header → LOG;
narrative → REFERENCE). Genuinely-open forks that remain:
...
- **Robotize → render-stack — DEFERRED BY CHOICE (2026-07-10):** recon complete, migration
  not licensed — CAPTURE-the-tape already freezes robotize as-heard. Full reasoning + the
  original 2026-06-12 bake-as-heard motivation → LOG.
...
- **(a2) Unified UNDO/REDO — BUILT** (Stage 2a/2b 2026-06-23/24; the SELECT+CLEAR action
  journal). Final text → LOG; codebase facts → REFERENCE.

- **(b) Trigger generators — SHIPPED as G3** (2026-07-07; independent trigger key-space —
  pitch + rhythm decoupled on one track). Final text → LOG. The register-unification
  question it surfaced is parked above.
...
**Retroactive CAPTURE — frame ring + live tape SHIPPED + by-ear GO 2026-06-20** (final
header → LOG; narrative → REFERENCE + `doc/plans/archive/2026-06-19-capture-ring.md`,
`doc/plans/archive/2026-06-20-unified-freeze.md`). Standing open threads:
...
`doc/plans/archive/2026-06-19-pre-build-recon.md`).** A deep multi-agent pass (8 subsystems, 122
...
**Per-track-RNG keystone — FIRST CUT SHIPPED + by-ear GO 2026-06-20** (final header → LOG;
narrative → REFERENCE).
...
**Phrase morphing — SHIPPED + by-ear GO 2026-06-16/18; KEPT (keep/cut resolved 2026-06-22).**
`make PHRASE_MORPH=0` survives as a ~6.5 KB main-RAM lever only (measured 6680 B). Final
text → LOG; narrative → REFERENCE.
...
**SWITCH-QUANTIZE — SHIPPED 2026-06-18** (global launch grid + auto-measured margin;
final header → LOG; narrative → REFERENCE).
...
**Phrase-recall freeze (~290 ms writeback wall) — CURED + by-ear GO 2026-06-22** (DRIFT-gated
writeback). Full investigation + the incremental-save fallback (relevant only if the
wander-survives-recall call ever reverses) → LOG.
...
- render double-buffer = 40 KB CCM; single-buffering saves 20 KB but loses lock-free safety —
  leave it. CPU: per-track processors run only when engaged. *(Stale worry corrected
  2026-07-11: the "force-dirty tracks re-render every tick, unmeasured at 16" open puzzle was
  RESOLVED 2026-06-26 — render change-detection makes a static field render ZERO; measured
  all-16 worst case incl. echo+robotize ≈ 27% CPU. See the LOG, 2026-06-26.)*
...
control surface — see the flat-map control-surface map, `doc/plans/archive/2026-06-21-system-flatmap.md`.)*
...
`doc/plans/archive/2026-06-21-system-flatmap.md` (Map A).
...
One-shot session plans stay in `~/.claude/plans/`. Plans are scaffolding — move to
`doc/plans/archive/` (or delete) once executed into this doc/code; only live plans
stay at the top level. The durable design home is *this file* plus
`MBSEQV4_DECISIONS_LOG.md` (dated chronology) and `OPEN_ITEMS.md` (defect/housekeeping
board); plans are the workflow that gets there. A root `CLAUDE.md` should carry a
short version of this convention so future sessions follow it.
