...
s32 SEQ_FILE_LoadAllFiles(u8 including_hw)
{
  ...
  // PHRASES: re-seed phrase occupancy from this session's MBSEQ_PH.V4 on disk so
  // a reloaded set lights up its captured phrases and they recall again (the
  // cross-session probe). Empty/un-captured slots still refuse recall.
  SEQ_PATTERN_ProbePhrasesOnLoad();
