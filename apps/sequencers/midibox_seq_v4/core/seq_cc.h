...
#define SEQ_CC_SLICE_GRID						0xA4 // bits 0..2: 0=off, 1..5 = 1/2/4/8/16-step slices (1stp added + values REMAPPED 2026-07-17 — clean dial order over stale-session compat, sessions are disposable); bit 7 = row bypass (config-preserving)
...
// Act 2 (plan 2026-07-17):
#define SEQ_CC_SLICE_CHOKE						0xA9 // 0..127 edge-cut amount: engaged slices lose the GLIDE tie at their final step (thermometer; cuts stop ringing across the new cut points)
#define SEQ_CC_SLICE_MOTION						0xAA // 0=frozen; 1..4 = re-roll the seeded fill every 8/4/2/1 bars (one detent per rate, the GRID selector idiom — was a 0..127 thermometer for a day, 32 clicks per zone read as "stuck") — animates the Seed axis only, skeleton + painted stay put
...
typedef struct {
  ...
  u8		slice_grid;              // slicer: bits 0..2 = grid (0=off, 1..5 = 1/2/4/8/16-step slices); bit 7 = bypass
  ...
  u8		slice_choke;             // slicer act 2: 0..127 edge-cut amount (glide tie cleared at choked slices' final step)
  u8		slice_motion;            // slicer act 2: 0=frozen; 1..4 = re-roll every 8/4/2/1 bars
...
// Neutralize the generative CC axis on a capture destination — two flavors,
// one per capture family (2026-07-18 capture-fidelity determination; full law
// in seq_cc.c above the implementation):
//   ForFlatten — dst is a RENDER-MIRROR copy (CaptureToSlot/ToTrack/ToSlotTrack
//     + the chord-index route): reset generation only, PRESERVE the
//     deterministic emission shapers (echo/LFO/direction/SUSTAIN/fx-dupl +
//     Delay/Roll/Nth/Root/Scale lanes) — they were never baked into the bytes.
//   ForTape — dst is a recording of the EMITTED stream (live tape / re-sim
//     via CaptureSpanPrepDst): the shapers are baked into the notes, reset
//     them too (the historical full reset).
// Identity (MIDI port/channel, event mode, drum mapping, step-data lanes) and
// structure (length, loop, clkdiv, groove, voicing, FORCE_SCALE) survive both.
// When you add a new SEQ_CC_*, classify it in the same review.
extern s32 SEQ_CC_ResetGenerativeForFlatten(u8 track);
extern s32 SEQ_CC_ResetGenerativeForTape(u8 track);
