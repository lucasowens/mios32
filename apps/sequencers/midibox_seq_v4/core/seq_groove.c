...
s32 SEQ_GROOVE_DelayGet(u8 track, u8 step)
{
  ...
  // check if within allowed range (disable = PROC-page live bypass, keeps the config)
  if( !groove || tcc->groove_style.disable ||
      groove >= (SEQ_GROOVE_NUM_PRESETS+SEQ_GROOVE_NUM_TEMPLATES) )
    return 0; // no groove
